...
@if($articles[0]!="")
    @foreach($articles as $article)
        <div class="blogs" data-scroll-reveal="enter bottom over 1s" >
            ...
            <span class="blogpic"><a href="{{ route('article',['id'=>$article->id]) }}" title="" target="_blank"><img src="{{ asset('uploads/avatar') }}/{{ $article->list_pic }}" style="height: 120px;" alt=""></a></span>
            <p class="blogtext">{{ $article->desc }} </p>
            <div class="bloginfo">
                <ul>
                    <li class="author"><a>{{ $author->name??'佚名' }}</a></li>
                    @if($article->category)
